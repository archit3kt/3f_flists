...
provider "grid" {
    mnemonics = "glance grief cave lab reopen buzz session quantum science garden tree harbor" 
    network = "test"
}
...
resource "grid_deployment" "test1" {
  ...
  ip_range = grid_network.netTest1.nodes_ip_range["95"]
  disks {
    name = "server1Disk"
    size = 5
  }
  ...
  vms {
    name = "server1"
    flist = "https://hub.grid.tf/archit3kt.3bot/archit3kt-k3s-1.22.8.flist"
    cpu = 2
    publicip = false
    planetary = false
    memory = 4096
